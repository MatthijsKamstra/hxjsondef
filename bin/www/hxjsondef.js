...
Hxjsondef.prototype = {
	convert: function(name,content) {
		...
		var key = this.typeDefMap.keys();
		while(key.hasNext()) {
			var key1 = key.next();
			var _this = this.typeDefMap;
			var arr = __map_reserved[key1] != null ? _this.getReserved(key1) : _this.h[key1];
			this.str += "\ntypedef " + key1 + " =\n{\n";
			var _g1 = 0;
			var _g = arr.length;
			while(_g1 < _g) {
	...
	,readJson: function(typeName,pjson,tab) {
		...
		if(pjson.length != null) {
			...
			while(_g1 < _g) {
				var i = _g1++;
				this.readJson(typeName,pjson[i],tab);
			}
			return;
		}
		var _g2 = 0;
		var _g11 = Reflect.fields(pjson);
		while(_g2 < _g11.length) {
			var varName = _g11[_g2];
			++_g2;
			var store = "";
			var c = "";
			if(this.isComment) {
			...
			var _g21 = Type["typeof"](Reflect.field(pjson,varName));
			switch(_g21[1]) {
			case 0:
			...
			case 6:
				switch(_g21[2]) {
				case Array:
					store = this.whatSortArray(varName,Reflect.field(pjson,varName),tab);
					break;
				case String:
	...
	,whatSortArray: function(varName,value,tab) {
		if(tab == null) {
			tab = "\t";
		}
		var xc = "";
